    ...
    enqueueDependencies(data, correlationId, options) {
        ...
        return new Promise((resolve, reject) => {
            ...
            self.templateDefinitionService.getDefinition(data[config.templateNameKey], correlationId, options) // get template definitions
                .then((definition) => { logger.info('1'); return definition.getDependencies(data, correlationId, options) }) // get dependencies
                .then((dependencies) => { logger.info('2'); return dependencyValidator.validate(dependencies) }) // validate dependency array
